...
class ControllerView {
  constructor() {
    this.ninePatchDrawer = new NinePatchAsciiDrawer(str => term(str), (x, y) => term.moveTo(x, y));
    this.faderLocations = {};
  ...
  drawFaderSlot(x, y, w, h) {
    ...
    this.ninePatchDrawer.draw(faderSlotPatch, x + slotHandleDiff, y, faderSlotWidth, h);
  }
  ...
  drawFaderHandle(x, y, w, h, parameter) {
    ...
    let handleY = Math.round(
      y + faderHandleMargin + (h - faderHandleMargin * 2 - faderHandleHeight) * ratio
    );

    for (let i = 0; i < h; i++) {
      if (i + y == handleY) {
        //draw current frame handle
        this.ninePatchDrawer.draw(faderHandlePatch, x, handleY, w, faderHandleHeight);
        i += faderHandleHeight - 1;
      } else {
        //erase previous frame's handle
        term.moveTo(x, i + y);
        term(' '.repeat(slotHandleDiff) + this.slotBuffer.buffer[i] + ' '.repeat(slotHandleDiff));
      }
    }
  }
  ...
  init(initialState) {
    ...
    //container
    const width = allFaders.length * (margin * 2 + faderWidth) + allFaders.length + 1; // faders + 2 margins per fader + dividers
  ...
  render(state) {
    ...
    for (let i = 0; i < allFaders.length; i++) {
      ...
      this.drawFaderHandle(location.x, location.y, faderWidth, faderHeight, parameter);
    }
  }
}
